package com.jyyl.guideapp.widget.wheelview;
...
public class WheelView extends View {
    ...
    private static final int[] SHADOWS_COLORS = new int[]{0x00000000, 0x00FFFFFF, 0x00FFFFFF};
    ...
    /**
     * 普通条目布局 StaticLayout 布局用于控制 TextView 组件, 一般情况下不会直接使用该组件, 除非你自定义一个组件 或者 想要直接调用
     * Canvas.drawText() 方法
     */
    ...
    /**
     * Scroll 类封装了滚动动作. 开发者可以使用 Scroll 或者 Scroll 实现类 去收集产生一个滚动动画所需要的数据, 返回一个急冲滑动的手势.
     * 该对象可以追踪随着时间推移滚动的偏移量, 但是这些对象不会自动向 View 对象提供这些位置. 如果想要使滚动动画看起来比较平滑, 开发者需要在适当的时机  获取 和 使用新的坐标;
     */
    ...
    /**
     * 设置当前元素的位置, 如果索引是错误的 不进行任何操作 -- 需要考虑该 WheelView 是否能循环 -- 根据是否需要滚动动画来确定是 ①滚动到目的位置 还是
     * ②晴空所有条目然后重绘
     * @param index
     *         要设置的元素索引值
     * @param animated
    ...
    /**
     * 获取 WheelView 是否可以循环 -- 如果可循环 : 第一个之前是最后一个, 最后一个之后是第一个; -- 如果不可循环 : 到第一个就不能上翻, 最后一个不能下翻
     * @return
     */
    ...
    private int calculateLayoutWidth(int widthSize, int mode) {
        ...
        if (maxLength > 0) {
        	 ...
        	 * Use java.lang.Math#ceil instead of android.util.FloatMath#ceil() since it is faster
        	  * as of API 8
        	 */
    ...
    private void createLayouts(int widthItems, int widthLabel) {
        ...
        if (itemsLayout == null || itemsLayout.getWidth() > widthItems) {
        	 ...
        	 * int width : 条目的宽度
        	 * Alignment align : Layout 的对齐方式, ALIGN_CENTER 居中对齐, ALIGN_NORMAL 左对齐, Alignment
        	 * .ALIGN_OPPOSITE 右对齐
    ...
    private SimpleOnGestureListener gestureListener = new SimpleOnGestureListener() {
         ...
         * (non-Javadoc)
         * @see android.view.GestureDetector.SimpleOnGestureListener#onScroll(android.view
         ...
         * 
         * (non-Javadoc)
         * @see android.view.GestureDetector.SimpleOnGestureListener#onFling(android.view
         * .MotionEvent, android.view.MotionEvent, float, float)
         */
